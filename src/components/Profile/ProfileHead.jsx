  ...
  return (
    <section className={`  md:w-[220px]  overflow-hidden box-border  text-white   text-center    `}>
      <div className='my-2 h-10 rounded-xl w-12 flex justify-center items-center  '>
        <button onClick={handleClick} className=' text-slate-200 text-sm md:text-xl hover:text-yellow-400 '><CgProfile /> </button>
      </div>
      {click ? <ProfileNavOpn status={status} click={click}/> :<></>}
    </section>
  )
}
